...
path_img_ex = 'ShanghaiTech/part_B/train_data/images/IMG_6.jpg'
...
path_gt_ex = "ShanghaiTech/part_B/train_data/ground-truth/GT_IMG_6.mat"
...
train_root_dir = "ShanghaiTech/part_B/train_data/"
...
test_root_dir = "ShanghaiTech/part_B/test_data/"
...
# ===== LOAD MODEL =====
model = MC_CNN().to(device)
model.load_state_dict(torch.load('crowd_counting.pth', map_location=device))
model.eval()
...
# ===== THRESHOLD (edit this if needed) =====
THRESHOLD = 50  # crowd vs non-crowd

all_preds = []
all_labels = []

with torch.no_grad():
    for imgs, _, gts in val_loader:
        imgs = imgs.to(device)
        ...
        pred_classes = (pred_counts > THRESHOLD).int()
        true_classes = (gts > THRESHOLD).int()

        all_preds.extend(pred_classes.numpy())
        all_labels.extend(true_classes.numpy())
...
with torch.no_grad():
    for imgs, _, gts in val_loader:
        ...
        all_pred_counts.extend(pred_counts.numpy())
        all_gt_counts.extend(gts.numpy())
...
# ===== 1. SCATTER PLOT =====
plt.figure()
plt.scatter(all_gt_counts, all_pred_counts)
...
# ===== 3. LINE PLOT =====
plt.figure()
plt.plot(all_gt_counts[:100], label="Ground Truth")
plt.plot(all_pred_counts[:100], label="Prediction")
plt.legend()
plt.title("Prediction vs Ground Truth (first 100 samples)")
plt.show()
